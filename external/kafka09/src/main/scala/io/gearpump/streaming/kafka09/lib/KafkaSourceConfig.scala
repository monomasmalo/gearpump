...
package io.gearpump.streaming.kafka09.lib

import java.util.Properties

import io.gearpump.util.LogUtil
import io.gearpump.streaming.kafka09.KafkaSource
import io.gearpump.streaming.kafka09.lib.grouper.{KafkaDefaultGrouper, KafkaGrouper}
 ...
 *
   ...
   * @return new KafkaConfig based on this but with [[io.gearpump.streaming.kafka09.lib.KafkaSourceConfig.CONSUMER_TOPICS]] set to given value
   */
   ...
   * the incoming queue size is above the [[io.gearpump.streaming.kafka09.lib.KafkaSourceConfig.FETCH_THRESHOLD]]
   * this is to set sleep interval
 *
   * @param sleepMS sleep interval in milliseconds
   * @return new KafkaConfig based on this but with [[io.gearpump.streaming.kafka09.lib.KafkaSourceConfig.FETCH_SLEEP_MS]] set to given value
   */
 ...
 *
   ...
   * @return new KafkaConfig based on this but with [[io.gearpump.streaming.kafka09.lib.KafkaSourceConfig.FETCH_THRESHOLD]] set to give value
   */
  ...
  /**
   *
   * [[io.gearpump.streaming.kafka09.lib.consumer.FetchThread]] stops fetching new messages if its incoming queue
   * size is above the threshold and starts again when the queue size is below it
 *
  ...
  /**
   * set [[io.gearpump.streaming.kafka09.lib.grouper.KafkaGrouper]], which
   * defines how kafka.common.TopicAndPartitions are mapped to source tasks
   *
   * @param className name of the factory class
   * @return new KafkaConfig based on this but with [[io.gearpump.streaming.kafka09.lib.KafkaSourceConfig.GROUPER_CLASS]] set to given value
   */
  ...
  /**
   * get [[io.gearpump.streaming.kafka09.lib.grouper.KafkaGrouper]] instance, which
   * defines how kafka.common.TopicAndPartitions are mapped to source tasks
 *
   * @return
   */
