...
object KafkaUtil {
  ...
  def getBroker(clientZk: => ZkClient, topic: String, partition: Int): Broker = {
    val zkClient = clientZk
    try {
      null
      /*
      val zku = new kafka.utils.ZkUtils(zkClient, zkConnection, false)
      val leader = zku.getLeaderForPartition(topic, partition)
  ...
  def connectZookeeper(config: ConsumerConfig): () => ZkClient = {
    ...
    () => new ZkClient(zookeeperConnect, sessionTimeout, connectionTimeout, null)
  }
