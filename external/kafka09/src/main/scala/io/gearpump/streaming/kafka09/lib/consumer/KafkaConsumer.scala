...
package io.gearpump.streaming.kafka09.lib.consumer
...
object KafkaConsumer {
  def apply(topic: String, partition: Int, startOffsetTime: Long, config: ConsumerConfig): KafkaConsumer = {
    val connectZk = null //KafkaUtil.connectZookeeper(config)
    val broker: kafka.cluster.Broker = null // KafkaUtil.getBroker(connectZk(), topic, partition)
    ...
    val b = broker.getBrokerEndPoint(SecurityProtocol.PLAINTEXT)
    val consumer = new SimpleConsumer(b.host, b.port, soTimeout, soBufferSize, clientId)
